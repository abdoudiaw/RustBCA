...
pub use crate::input::{GeometryInput, Input0D, Input1D, Input2D, InputFile, Options};
pub use crate::output::OutputUnits;
pub use crate::sphere::{InputSphere, Sphere, SphereInput};
pub use crate::structs::*;

#[cfg(feature = "parry3d")]
pub use crate::parry::{
    InputParryBall, InputParryTriMesh, ParryBall, ParryBallInput, ParryTriMesh, ParryTriMeshInput,
};
#[cfg(feature = "parry3d")]
pub use parry3d_f64::na::{Matrix3, Point3, Rotation3, Vector3};
...
pub extern "C" fn compound_tagged_bca_list_c(input: InputTaggedBCA) -> OutputTaggedBCA {
    ...
    let mut total_output = total_output.into_boxed_slice();
    let mut output_tags = output_tags.into_boxed_slice();
    let mut output_weights = output_weights.into_boxed_slice();
    let mut output_incident = output_incident.into_boxed_slice();
...
#[no_mangle]
pub extern "C" fn reflect_single_ion_c(
...
#[no_mangle]
pub extern "C" fn compound_bca_list_fortran(
    num_incident_ions: &mut c_int,
    track_recoils: &mut bool,
    ux: *mut f64,
    uy: *mut f64,
    uz: *mut f64,
    E1: *mut f64,
    ...
    for (((((((E1_, ux_), uy_), uz_), Z1_), Ec1_), Es1_), m1_) in E1
        .iter()
        .zip(ux)
        .zip(uy)
        .zip(uz)
...
pub extern "C" fn simple_bca_c(
    ...
    let output = simple_bca(
        x, y, z, ux, uy, uz, E1, Z1, m1, Ec1, Es1, Z2, m2, Ec2, Es2, n2, Eb2,
    );
...
pub fn compound_bca_list_py(
    ...
    assert_eq!(
        ux.len(),
        num_incident_ions,
        "Input error: list of x-directions is not the same length as list of incident energies."
    );
    assert_eq!(
        uy.len(),
        num_incident_ions,
        "Input error: list of y-directions is not the same length as list of incident energies."
    );
    assert_eq!(
        uz.len(),
        num_incident_ions,
        "Input error: list of z-directions is not the same length as list of incident energies."
    );
    ...
    assert_eq!(
        m2.len(),
        num_species_target,
        "Input error: list of target atomic masses is not the same length as atomic numbers."
    );
    assert_eq!(
        Ec2.len(),
        num_species_target,
        "Input error: list of target cutoff energies is not the same length as atomic numbers."
    );
    ...
    assert_eq!(
        n2.len(),
        num_species_target,
        "Input error: list of target number densities is not the same length as atomic numbers."
    );
    ...
    let mut index: usize = 0;
    for (energy, ux_, uy_, uz_, Z1_, Ec1_, Es1_, m1_) in
        izip!(energies, ux, uy, uz, Z1, Ec1, Es1, m1)
    {
        let mut energy_out;

        let p =
            particle::Particle::default_incident(m1_, Z1_, energy, Ec1_, Es1_, x, ux_, uy_, uz_);
...
#[pyfunction]
pub fn compound_bca_list_tracked_py(
    energies: Vec<f64>,
    ux: Vec<f64>,
    uy: Vec<f64>,
    uz: Vec<f64>,
    ...
    let incident_particles: Vec<particle::Particle> = izip!(energies, ux, uy, uz, Z1, Ec1, Es1, m1)
        ...
        .map(|(index, (energy, ux_, uy_, uz_, Z1_, Ec1_, Es1_, m1_))| {
            let mut p = particle::Particle::default_incident(
                m1_, Z1_, energy, Ec1_, Es1_, x, ux_, uy_, uz_,
            );
            p.tag = index as i32;
            p
        })
    ...
    for particle in finished_particles {
        if (particle.left) | (particle.incident) {
            ...
            let energy_out = if particle.stopped {
                0.
            } else {
                particle.E / EV
            };
...
pub fn reflect_single_ion_py(
    ...
    let Z1 = unpack(
        ion.get_item("Z")
            .expect("Cannot get ion Z from dictionary. Ensure ion['Z'] exists."),
    );
    let m1 = unpack(
        ion.get_item("m")
            .expect("Cannot get ion mass from dictionary. Ensure ion['m'] exists."),
    );
    let Ec1 = unpack(
        ion.get_item("Ec")
            .expect("Cannot get ion cutoff energy from dictionary. Ensure ion['Ec'] exists."),
    );
    let Es1 =
        unpack(ion.get_item("Es").expect(
            "Cannot get ion surface binding energy from dictionary. Ensure ion['Es'] exists.",
        ));

    let Z2 = unpack(
        target
            .get_item("Z")
            .expect("Cannot get target Z from dictionary. Ensure target['Z'] exists."),
    );
    let m2 = unpack(
        target
            .get_item("m")
            .expect("Cannot get target mass from dictionary. Ensure target['m'] exists."),
    );
    let Ec2 =
        unpack(target.get_item("Ec").expect(
            "Cannot get target cutoff energy from dictionary. Ensure target['Ec'] exists.",
        ));
    let Es2 = unpack(target.get_item("Es").expect(
        "Cannot get target surface binding energy from dictionary. Ensure target['Es'] exists.",
    ));
    let Eb2 = unpack(target.get_item("Eb").expect(
        "Cannot get target bulk binding energy from dictionary. Ensure target['Eb'] exists.",
    ));
    let n2 = unpack(
        target
            .get_item("n")
            .expect("Cannot get target density from dictionary. Ensure target['n'] exists."),
    );

    assert!(
        vx > 0.0,
        "Input error: vx must be greater than zero for incident particles to hit surface at x=0."
    );
    ...
    let p = particle::Particle::default_incident(m1, Z1, energy_eV, Ec1, Es1, x, ux, uy, uz);
...
#[pyfunction]
pub fn compound_bca_list_1D_py(
    ux: Vec<f64>,
    uy: Vec<f64>,
    uz: Vec<f64>,
    energies: Vec<f64>,
    ...
    let incident_particles: Vec<particle::Particle> = izip!(energies, ux, uy, uz, Z1, Ec1, Es1, m1)
        .map(|(energy, ux_, uy_, uz_, Z1_, Ec1_, Es1_, m1_)| {
            particle::Particle::default_incident(m1_, Z1_, energy, Ec1_, Es1_, x, ux_, uy_, uz_)
        })
        .collect();
...
pub fn simple_bca(
    ...
    assert!(
        E1 > 0.0,
        "Error: Incident energy cannot be less than or equal to 0."
    );
    assert!(
        Ec1 > 0.0,
        "Error: Cutoff energy Ec1 cannot be less than or equal to 0."
    );
    assert!(
        Ec2 > 0.0,
        "Error: Cutoff energy Ec2 cannot be less than or equal to 0."
    );
    ...
    output
        .iter()
        .filter(|particle| (particle.incident) | (particle.left))
        .map(|particle| {
            [
                particle.Z,
                particle.m / AMU,
...
#[cfg(feature = "parry3d")]
#[no_mangle]
pub extern "C" fn rotate_given_surface_normal(
    nx: f64,
    ny: f64,
    nz: f64,
    ux: &mut f64,
...
pub fn rotate_given_surface_normal_py(
    nx: f64,
    ny: f64,
    nz: f64,
    ux: f64,
...
pub fn rotate_given_surface_normal_vec_py(
    nx: Vec<f64>,
    ny: Vec<f64>,
    nz: Vec<f64>,
    ux: Vec<f64>,
    ...
    (0..length).into_iter().for_each(|index| {
        ...
        rotate_given_surface_normal(
            nx[index], ny[index], nz[index], &mut ux_, &mut uy_, &mut uz_,
        );
        ux_new.push(ux_);
        uy_new.push(uy_);
        uz_new.push(uz_);
...
pub fn rotate_back_vec_py(
    ...
    let (ux_new, (uy_new, uz_new)) = (nx, ny, nz, ux, uy, uz)
        .into_par_iter()
        .map(|(nx_, ny_, nz_, ux_, uy_, uz_)| {
            let mut ux_mut = ux_;
            let mut uy_mut = uy_;
            let mut uz_mut = uz_;
...
fn unpack(python_float: &PyAny) -> f64 {
    python_float
        .downcast::<PyFloat>()
        .expect("Error unpacking Python float to f64. Check values.")
        .value()
}
...
pub fn sputtering_yield(
    ...
    assert!(
        angle.abs() <= 90.0,
        "Incident angle w.r.t. surface normal, {}, cannot exceed 90 degrees.",
        angle
    );
    ...
    (0..num_samples as u64).into_par_iter().for_each(|index| {
        let p = particle::Particle::default_incident(m1, Z1, energy, Ec1, Es1, x, ux, uy, uz);
...
pub fn reflection_coefficient(
    ...
    (
        ...
        energy_reflected / EV / energy / num_samples as f64,
    )
}
...
///     R_N (f64): reflection coefficient (number of particles reflected / number of incident particles)
///     R_E (f64): energy reflection coefficient (sum of reflected particle energies / total incident energy)
pub fn compound_reflection_coefficient(
    ion: &PyDict,
    targets: Vec<&PyDict>,
    target_number_densities: Vec<f64>,
    energy: f64,
    angle: f64,
    num_samples: usize,
) -> (f64, f64) {
    ...
    let Z2: Vec<f64> = targets
        .iter()
        .enumerate()
        .map(|(index, item)| {
            unpack(item.get_item("Z").unwrap_or_else(|| {
                panic!(
                    "Cannot get target Z from dictionary at index {}. Ensure target['Z'] exists.",
                    index
                )
            }))
        })
        .collect();
    let m2: Vec<f64> = targets
        .iter()
        .enumerate()
        .map(|(index, item)| {
            unpack(item.get_item("m").unwrap_or_else(|| {
                panic!(
                    "Cannot get target m from dictionary at index {}. Ensure target['m'] exists.",
                    index
                )
            }))
        })
        .collect();
    let Ec2: Vec<f64> = targets
        .iter()
        .enumerate()
        .map(|(index, item)| {
            unpack(item.get_item("Ec").unwrap_or_else(|| {
                panic!(
                    "Cannot get target Ec from dictionary at index {}. Ensure target['Ec'] exists.",
                    index
                )
            }))
        })
        .collect();
    let Es2: Vec<f64> = targets
        .iter()
        .enumerate()
        .map(|(index, item)| {
            unpack(item.get_item("Es").unwrap_or_else(|| {
                panic!(
                    "Cannot get target Es from dictionary at index {}. Ensure target['Es'] exists.",
                    index
                )
            }))
        })
        .collect();
    let Eb2: Vec<f64> = targets
        .iter()
        .enumerate()
        .map(|(index, item)| {
            unpack(item.get_item("Eb").unwrap_or_else(|| {
                panic!(
                    "Cannot get target Eb from dictionary at index {}. Ensure target['Eb'] exists.",
                    index
                )
            }))
        })
